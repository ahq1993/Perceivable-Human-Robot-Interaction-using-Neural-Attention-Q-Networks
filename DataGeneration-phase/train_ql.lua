...
local screen, depth, reward,human, terminal = perform_action('-',-1,-1)
...
function generate_data(episode)
	...
	local step=1
			while step <t_steps do
				...
				if reward>15 then
					action_index,px,py = agent:perceive(1, screen,depth,human, terminal, false, numSteps,step,0.1)
				else
					action_index,px,py = agent:perceive(0, screen,depth,human, terminal, false, numSteps,step,0.1)
            end
				step=step+1		
				print('action_index')
				print(action_index)
				...
				if not terminal then 
					screen,depth,reward,human,terminal=perform_action(aset[action_index],px,py)
				else  
					screen,depth, reward, human, terminal = perform_action('-',-1,-1)
				end
